def serverConfig = [
    "test": [
        ...
        appHome: "/home/alex/alex-api",
        profile: "test",
        appPort: "9000",
        containerPort: "8888",
        deployLogDir: "/home/alex/alex-api/logs",
        appNetwork: "network",
        containerName: "alex-api",
        appJavaToolOptions: "-Xms64m -Xmx160m"
    ],
    "prod": [
        ip: "47.119.185.39",
        user: "alex",
        ...
        appPort: "9000",
        containerPort: "8888",
        deployLogDir: "/home/alex-prod/alex-api/logs",
        appNetwork: "network",
        containerName: "alex-api",
        appJavaToolOptions: "-Xms64m -Xmx160m"
    ]
...
pipeline {
    ...
    environment {
        ...
        REGISTRY_REPO = "alex_zcz/alex-api"
    ...
    stages {
        stage('Init') {
            ...
            steps {
                script {
                    ...
                    env.DEPLOY_PROFILE = cfg.profile
                    env.DEPLOY_APP_PORT = cfg.appPort
                    env.DEPLOY_CONTAINER_PORT = cfg.containerPort
                    env.DEPLOY_LOG_DIR = cfg.deployLogDir
                    env.DEPLOY_NETWORK = cfg.appNetwork
                    env.DEPLOY_CONTAINER_NAME = cfg.containerName
                    env.DEPLOY_APP_JAVA_TOOL_OPTIONS = cfg.appJavaToolOptions
                    env.DEPLOY_HEALTH_URL = "http://${cfg.ip}:${cfg.appPort}/actuator/health"
                }
            }
        }
        ...
        stage('Deploy') {
            agent any
            steps {
                checkout scm   // 从 main 分支脚本仓库中拉取 docker-compose.yml
                script {
                    withCredentials([usernamePassword(credentialsId: env.REGISTRY_CREDENTIALS_ID, usernameVariable: 'REG_USER', passwordVariable: 'REG_PASS')]) {
                        sshagent([env.DEPLOY_CRED]) {
                            sh """
                                ...
                                ssh ${env.DEPLOY_USER}@${env.DEPLOY_IP} "mkdir -p ${env.DEPLOY_HOME} ${env.DEPLOY_LOG_DIR}"
                                ...
                                'SPRING_PROFILES_ACTIVE=${env.DEPLOY_PROFILE}' \
                                'DEPLOY_LOG_DIR=${env.DEPLOY_LOG_DIR}' \
                                'APP_NETWORK=${env.DEPLOY_NETWORK}' \
                                'APP_JAVA_TOOL_OPTIONS=${env.DEPLOY_APP_JAVA_TOOL_OPTIONS}' \
                                > ${env.DEPLOY_HOME}/.env"
